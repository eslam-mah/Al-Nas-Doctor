...
class _PendingRequestsList extends StatelessWidget {
  ...
  Future<void> _onRefresh(BuildContext context) async {
    ...
    // Also refresh chat list so both tabs stay in sync
    context.read<GetChatListCubit>().getChatList();
  }
...
class _ChatList extends StatelessWidget {
  ...
  Future<void> _onRefresh(BuildContext context) async {
    context.read<GetChatListCubit>().getChatList();
    // Also refresh requests so both tabs stay in sync
    context.read<GetPendingRequestsCubit>().getPendingRequests();
  }

  @override
  Widget build(BuildContext context) {
    return BlocListener<CloseChatCubit, CloseChatState>(
      listener: (context, state) {
        if (state is CloseChatSuccess) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              backgroundColor: AlNasTheme.blue100,
              behavior: SnackBarBehavior.floating,
              shape: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(12),
              ),
            ...
            SnackBar(
              content: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(state.failure.message ?? 'Error closing chat'),
                  ),
                ],
              ),
              backgroundColor: AlNasTheme.error,
...
// ══════════════════════════════════════════════════════════════════
// SHARED UI: Scrollable Empty State (needed for RefreshIndicator)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// SHARED UI: Scrollable Error State (needed for RefreshIndicator)
// ══════════════════════════════════════════════════════════════════
class _ScrollableErrorState extends StatelessWidget {
  final String message;
  final IconData icon;

  const _ScrollableErrorState({required this.message, required this.icon});
